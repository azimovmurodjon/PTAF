 ...
 * <p>
 * Enhancements:
 *  - Role locators now support optional name (unnamed roles) safely.
 *  - Chained locators now support ID/NAME/CLASS off a base Locator.
 *  - 100% backward compatible with "TYPE_value" patterns like "ROW_rowElement > Button_buttonName".
 */
public class LocatorHandler {
    ...
    private boolean isUnnamed(String locator, String locatorType) {
        return locator == null
                || locator.isEmpty()
                || locator.equalsIgnoreCase(locatorType); // safety if parser returned "Button"
    }
    ...
    public Locator getLocatorForType(String locatorType, Page page, String locator) {
        ...
        switch (t) {
            ...
            case "BUTTONSUBMIT":
                // Preserve your existing "pressed" semantics if name is given; otherwise unnamed
                return isUnnamed(locator, t)
                        ? page.getByRole(AriaRole.BUTTON)
            ...
            case "SLIDER":
                return isUnnamed(locator, t)
                        ? page.getByRole(AriaRole.SLIDER)
            ...
            case "MENUITEMCHECKBOX":
                return isUnnamed(locator, t)
                        ? page.getByRole(AriaRole.MENUITEMCHECKBOX)
            ...
            case "SEPARATOR":
                return isUnnamed(locator, t)
                        ? page.getByRole(AriaRole.SEPARATOR)
            ...
            case "ROLE":
                // If you use ROLE_Button etc., unnamed means raw role
                return isUnnamed(locator, t)
                        ? page.getByRole(AriaRole.valueOf(locatorType.toUpperCase()))
                        : page.getByRole(AriaRole.valueOf(locatorType.toUpperCase()));
    ...
    public Locator getLocatorForType(String locatorType, FrameLocator frame, String locator) {
        ...
        switch (t) {
            ...
            case "SLIDER":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.SLIDER)
                        : frame.getByRole(AriaRole.SLIDER, new FrameLocator.GetByRoleOptions().setName(locator));

            case "SPINBUTTON":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.SPINBUTTON)
                        : frame.getByRole(AriaRole.SPINBUTTON, new FrameLocator.GetByRoleOptions().setName(locator));

            case "PROGRESSBAR":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.PROGRESSBAR)
                        : frame.getByRole(AriaRole.PROGRESSBAR, new FrameLocator.GetByRoleOptions().setName(locator));

            case "ALERT":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.ALERT)
                        : frame.getByRole(AriaRole.ALERT, new FrameLocator.GetByRoleOptions().setName(locator));

            case "ALERTDIALOG":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.ALERTDIALOG)
                        : frame.getByRole(AriaRole.ALERTDIALOG, new FrameLocator.GetByRoleOptions().setName(locator));

            case "DIALOG":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.DIALOG)
                        : frame.getByRole(AriaRole.DIALOG, new FrameLocator.GetByRoleOptions().setName(locator));

            case "NAVIGATION":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.NAVIGATION)
                        : frame.getByRole(AriaRole.NAVIGATION, new FrameLocator.GetByRoleOptions().setName(locator));

            case "MENU":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.MENU)
                        : frame.getByRole(AriaRole.MENU, new FrameLocator.GetByRoleOptions().setName(locator));

            case "MENUITEM":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.MENUITEM)
                        : frame.getByRole(AriaRole.MENUITEM, new FrameLocator.GetByRoleOptions().setName(locator));

            case "MENUITEMCHECKBOX":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.MENUITEMCHECKBOX)
                        : frame.getByRole(AriaRole.MENUITEMCHECKBOX, new FrameLocator.GetByRoleOptions().setName(locator));

            case "MENUITEMRADIO":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.MENUITEMRADIO)
                        : frame.getByRole(AriaRole.MENUITEMRADIO, new FrameLocator.GetByRoleOptions().setName(locator));

            case "TREE":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.TREE)
                        : frame.getByRole(AriaRole.TREE, new FrameLocator.GetByRoleOptions().setName(locator));

            case "TREEITEM":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.TREEITEM)
                        : frame.getByRole(AriaRole.TREEITEM, new FrameLocator.GetByRoleOptions().setName(locator));

            case "GRID":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.GRID)
                        : frame.getByRole(AriaRole.GRID, new FrameLocator.GetByRoleOptions().setName(locator));

            case "GRIDCELL":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.GRIDCELL)
                        : frame.getByRole(AriaRole.GRIDCELL, new FrameLocator.GetByRoleOptions().setName(locator));

            case "SEPARATOR":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.SEPARATOR)
                        : frame.getByRole(AriaRole.SEPARATOR, new FrameLocator.GetByRoleOptions().setName(locator));

            case "SWITCH":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.SWITCH)
                        : frame.getByRole(AriaRole.SWITCH, new FrameLocator.GetByRoleOptions().setName(locator));

            case "STATUS":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.STATUS)
                        : frame.getByRole(AriaRole.STATUS, new FrameLocator.GetByRoleOptions().setName(locator));

            case "BANNER":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.BANNER)
                        : frame.getByRole(AriaRole.BANNER, new FrameLocator.GetByRoleOptions().setName(locator));

            case "FOOTER":
            case "CONTENTINFO":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.CONTENTINFO)
                        : frame.getByRole(AriaRole.CONTENTINFO, new FrameLocator.GetByRoleOptions().setName(locator));

            case "MAIN":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.MAIN)
                        : frame.getByRole(AriaRole.MAIN, new FrameLocator.GetByRoleOptions().setName(locator));

            case "COMPLEMENTARY":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.COMPLEMENTARY)
                        : frame.getByRole(AriaRole.COMPLEMENTARY, new FrameLocator.GetByRoleOptions().setName(locator));

            case "REGION":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.REGION)
                        : frame.getByRole(AriaRole.REGION, new FrameLocator.GetByRoleOptions().setName(locator));

            case "ARTICLE":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.ARTICLE)
                        : frame.getByRole(AriaRole.ARTICLE, new FrameLocator.GetByRoleOptions().setName(locator));

            case "FORM":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.FORM)
                        : frame.getByRole(AriaRole.FORM, new FrameLocator.GetByRoleOptions().setName(locator));

            case "LOG":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.LOG)
                        : frame.getByRole(AriaRole.LOG, new FrameLocator.GetByRoleOptions().setName(locator));

            case "MARQUEE":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.MARQUEE)
                        : frame.getByRole(AriaRole.MARQUEE, new FrameLocator.GetByRoleOptions().setName(locator));

            case "TIMER":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.TIMER)
                        : frame.getByRole(AriaRole.TIMER, new FrameLocator.GetByRoleOptions().setName(locator));

            case "TOOLTIP":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.TOOLTIP)
                        : frame.getByRole(AriaRole.TOOLTIP, new FrameLocator.GetByRoleOptions().setName(locator));

            case "TOOLBAR":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.TOOLBAR)
                        : frame.getByRole(AriaRole.TOOLBAR, new FrameLocator.GetByRoleOptions().setName(locator));

            case "PRESENTATION":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.PRESENTATION)
                        : frame.getByRole(AriaRole.PRESENTATION, new FrameLocator.GetByRoleOptions().setName(locator));

            case "FIGURE":
                return isUnnamed(locator, t)
                        ? frame.getByRole(AriaRole.FIGURE)
                        : frame.getByRole(AriaRole.FIGURE, new FrameLocator.GetByRoleOptions().setName(locator));

            case "TEXT":
                return frame.getByText(locator);

            case "ROLE":
                return frame.getByRole(AriaRole.valueOf(locatorType.toUpperCase()));
    ...
    public Locator getLocatorForType(String locatorType, Locator baseLocator, String locator) {
        ...
        switch (t) {
            ...
            case "ROLE":
                return baseLocator.getByRole(AriaRole.valueOf(locatorType.toUpperCase()));
            ...
            // NEW: allow chaining simple CSS-style shortcuts
            case "ID":
                return baseLocator.locator("#" + locator);
